...
class DNSQuestion(DNSEntry):
    ...
    def answeredBy(self, rec):
        """Return true if the question is answered by the record."""
...
class DNSRecord(DNSEntry):
    ...
    def suppressedByAnswer(self, other):
        """Return true if another record has same name, type and class.
    ...
    def getExpirationTime(self, percent):
        """Return the time at which this record will have expired.
    ...
    def isStale(self, now):
        """Return true if this record is at least half way expired."""
        return self.getExpirationTime(50) <= now

    def resetTTL(self, other):
        """Set this record's TTL and created time to that of.
...
class DNSService(DNSRecord):
    ...
    def __eq__(self, other):
        """Test equality on priority, weight, port and server."""
...
class DNSIncoming(object):
    ...
    def readString(self, length):
        """Read a string of a given length from the packet."""
    ...
    def readOthers(self):
        """Read the answers, authorities and additionals section.
        ...
        for i in xrange(n):
            ...
            if type == _TYPE_A:
                ...
                rec = DNSText(domain, type, clazz, ttl,
    ...
    def readUTF(self, offset, length):
        """Read a UTF-8 string of a given length from the packet."""
    ...
    def readName(self):
        ...
        while True:
            ...
            if t == 0x00:
                ...
                if off >= first:
                    text = "Bad domain name (circular) at " + str(off)
...
class DNSOutgoing(object):
    ...
    def addAnswerAtTime(self, record, now):
        """Add an answer if if does not expire by a certain time."""
        if record is not None:
            if now == 0 or not record.isExpired(now):
                self.answers.append((record, now))

    def addAuthorativeAnswer(self, record):
        """Add an authoritative answer."""
    ...
    def insertShort(self, index, value):
        """Insert an unsigned short in a certain position in the packet."""
        self.data.insert(index, struct.pack('!H', value))
        self.size += 2

    def writeShort(self, value):
        """Write an unsigned short to the packet."""
        self.pack('!H', value)

    def writeInt(self, value):
        """Write an unsigned integer to the packet."""
    ...
    def writeUTF(self, s):
        """Write a UTF-8 string of a given length to the packet."""
    ...
    def writeName(self, name):
        """Write a domain name to the packet."""
    ...
    def packet(self):
        """Return a string containing the packet's bytes.
...
class DNSCache(object):
    ...
    def get(self, entry):
        """Get an entry by key.  Will return None.
    ...
    def getByDetails(self, name, type, clazz):
        """Get an entry by details.

        Will return None if there is no matching entry.
        """
        entry = DNSEntry(name, type, clazz)
        return self.get(entry)

    def entriesWithName(self, name):
        """Return a list of entries whose key matches the name."""
        try:
...
class Engine(threading.Thread):
    """An engine wraps read access to sockets, allowing objects.

    that need to receive data from sockets to be called back when the
    sockets are ready.
...
class Listener(object):
    """A Listener is used by this module.

    to listen on the multicast group to which DNS messages are sent,
    allowing the implementation to cache information as it arrives.
...
class Reaper(threading.Thread):
    """A Reaper is used by this module to remove cache entries.
...
class ServiceBrowser(threading.Thread):
    ...
    def __init__(self, zc, type, listener):
        """Create a browser for a specific type."""
        threading.Thread.__init__(self)
        self.zc = zc
        self.type = type
        ...
        if record.type == _TYPE_PTR and record.name == self.type:
            ...
            try:
                ...
                if not expired:
                    ...
                    callback = \
                        lambda x: self.listener.removeService(x,
                                                              self.type,
                                                              record.alias)
                ...
                if not expired:
                    ...
                    callback = \
                        lambda x: self.listener.addService(x,
                                                           self.type,
                                                           record.alias)
                    self.list.append(callback)
...
class ServiceInfo(object):
    ...
    def setProperties(self, properties):
        """Set properties and text of this info from a dictionary."""
    ...
    def setText(self, text):
        """Set properties and text given a text field."""
        self.text = text
        try:
            result = {}
            end = len(text)
    ...
    def updateRecord(self, zc, now, record):
        """Update service information from a DNS record."""
            ...
            if record.type == _TYPE_A:
                ...
                if record.name == self.name:
                    ...
                    self.updateRecord(zc, now,
                                      zc.cache.getByDetails(self.server,
                                                            _TYPE_A,
                                                            _CLASS_IN))
    ...
    def request(self, zc, timeout):
        """Return true if the service could be discovered.

        on the network, and updates this object with details discovered.
        """
        now = currentTimeMillis()
...
class Zeroconf(object):
    ...
    def __init__(self, bindaddress=None):
        """Create an instance of the Zeroconf class.

        establishing multicast communications, listening and reaping threads.
        """
        global _GLOBAL_DONE
        _GLOBAL_DONE = False
    ...
    def wait(self, timeout):
        """Calling thread waits for a given number.

        of milliseconds or until notified.
        """
        self.condition.acquire()
    ...
    def notifyAll(self):
        """Notifie all waiting threads."""
        self.condition.acquire()
        self.condition.notifyAll()
        self.condition.release()

    def getServiceInfo(self, type, name, timeout=3000):
        """Return network's service information.

        for a particular name and type, or None if no service
        matches by the timeout, which defaults to 3 seconds.
        """
    ...
    def addServiceListener(self, type, listener):
        """Add a listener for a particular service type.

        This object will then have its updateRecord method
        called when information arrives for that type.
        """
        self.removeServiceListener(listener)
        self.browsers.append(ServiceBrowser(self, type, listener))

    def removeServiceListener(self, listener):
        """Remove a listener from the set that is currently listening."""
    ...
    def registerService(self, info, ttl=_DNS_TTL):
        """Register service information to the network.

        with a default TTL of 60 seconds.  Zeroconf will then
        respond to requests for information for that service.
        The name of the service may be
    ...
    def checkService(self, info):
        """Check the network for a unique service name.

        modifying the ServiceInfo passed in if it is not unique.
        """
        now = currentTimeMillis()
        nextTime = now
        i = 0
    ...
    def addListener(self, listener, question):
        """Add a listener for a given question.
    ...
    def updateRecord(self, now, rec):
        """Used to notify listeners of new information.

        that has updated a record.
        """
    ...
    def handleResponse(self, msg):
        ...
        All answers are held in the cache, and listeners are notified.
        """
        now = currentTimeMillis()
        for record in msg.answers:
            expired = record.isExpired(now)
    ...
    def close(self):
        """End the background threads.

        and prevent this instance from servicing further queries.
        """
        global _GLOBAL_DONE
        if not _GLOBAL_DONE:
            _GLOBAL_DONE = True
